	...
	inline bool hit_test(const Ray& ray, HitRecord& rec) const {
		...
		if (abs(ray.direction.z()) > 1e-8) {
			...
			if (this->x0 <= p.x() && p.x() <= this->x1  &&
				...
				rec.cos_ray_normal = abs(rec.normal.dot( ray.direction ));
				hit_u_v(rec);
				return true;
			}
		}
...
struct XZ_Rectangle : public Hitable {
	float x0, z0;
	float x1, z1;
	float y;
	XZ_Rectangle() {}
	XZ_Rectangle(const float & x0, const float & z0,
		const float & x1, const float & z1, const float & y) :
		x0(x0), z0(z0), x1(x1), z1(z1), y(y) {}

	//TODO have not handle normal for rectangle

	inline float area() const {
		return abs((x1 - x0) * (z1 - z0));
	}
	...
	inline void hit_u_v(HitRecord& rec) const {
		...
		// hit_point's must be between x0,x1   y is the same
		rec.v = (rec.hit_point.x() - x0) / (x1 - x0);
		rec.u = (rec.hit_point.z() - z0) / (z1 - z0);
	}
	...
	inline bool hit_test(const Ray& ray, HitRecord& rec) const {
		...
		//  y = y0 + bt
		//  z = z0 + ct
		//  since y is fixed, then we can get t, then we get x and y, judge the range of x, y with hit point
		//  
		const float T_MIN = 1e-2f;
		const float T_MAX = 1e20f;
		if (abs(ray.direction.y()) > 1e-8) {
			float t = (this->y - ray.from.y()) / ray.direction.y();
			
			if (t < T_MIN || t > T_MAX) return false;
			auto p = ray.point_at(t);
			if (this->x0 <= p.x() && p.x() <= this->x1  &&
				this->z0 <= p.z() && p.z() <= this->z1) {

				//cout << p << " |" << this->x0 << " : " << this->x1 << endl;

				rec.hit_point = p;
				rec.t = t;
				float y = ray.direction.y() > 0 ? -1.0f : 1.0f;
				rec.normal = Vec3(0, y, 0);
	...
	static const int thickness = 1e-8f;
	virtual AABB bounding_box() const {
		AABB aabb = AABB({ x0,z0,y - thickness }, { x1,z1,y + thickness });
		return aabb;
	}
